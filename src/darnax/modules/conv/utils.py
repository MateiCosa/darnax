...
def conv_backward_with_dual_threshold(
    x: Array,
    y: Array,
    y_hat: Array,
    threshold_hebb: Array,
    threshold_anti: Array,
    anti_strength: Array,
    kernel_shape: tuple[int, int],
    groups: int = 1,
    strides: IntPair = (1, 1),
    ...
    """Kernel gradient with Hebbian gate and anti-Hebbian gate.

    Extends ``conv_backward_with_threshold`` with a second, lower threshold
    that applies a negative (anti-Hebbian) update to neurons whose stability
    margin is deeply negative.

    The stability margin at position (n,h,w,c) is  ``m = y[n,h,w,c] * y_hat[n,h,w,c]``.

    Update per output position::

        Hebbian   : applied where  m < threshold_hebb   (margins below κ)
        Anti-Hebb : applied where  m < threshold_anti   (margins below κ⁻ < 0)

    Combined signed mask::

        net = 𝟙(m < threshold_hebb) − anti_strength · 𝟙(m < threshold_anti)
    ...
    x : Array
        Input tensor of shape (N, H, W, Cin).
    y : Array
        Post-synaptic state, shape (N, Ho, Wo, Cout).
    y_hat : Array
        Aggregated field at receiver, shape (N, Ho, Wo, Cout).
    threshold_hebb : scalar Array
        Upper threshold κ > 0.  Hebbian gate: ``m < threshold_hebb``.
    threshold_anti : scalar Array
        Lower threshold κ⁻ < 0.  Anti-Hebbian gate: ``m < threshold_anti``.
    anti_strength : scalar Array
        Relative weight α of the anti-Hebbian term.
    kernel_shape, groups, strides, padding_mode, lhs_dilation, rhs_dilation
        Same as ``conv_backward_with_threshold``.

    Returns
    -------
    Array
        Signed, variance-normalised kernel gradient of shape (Kh, Kw, Cin, Cout).
    """
    n, ho, wo, Cout = y.shape
    kh, kw = kernel_shape
    Cin = x.shape[-1]
    ...
    margin = y * y_hat
    mask_hebb = (margin < threshold_hebb).astype(y.dtype)
    mask_anti = (margin < threshold_anti).astype(y.dtype)
    signed_mask = mask_hebb - anti_strength * mask_anti
    out_masked = y * signed_mask
